...
4. The flippers are controlled by directly setting the angle to the plugin.  
...
- for 2022RoboCup (robot in <strong>.xacro</strong> format):  
`roslaunch nubot_launch pumbaa_2022RoboCup.launch`  
- for robot in <strong>.xacro</strong> format:  
`roslaunch nubot_launch pumbaa_xacro.launch`  
- for robot in <strong>.sdf</strong> format:  
`roslaunch nubot_launch pumbaa_sdf.launch`  
4. to display robot in rviz: `roslaunch nubot_launch rviz_pumbaa.launch`
...
For LiDAR in <strong>.sdf</strong> format, need to change "Fixed Frame" in Rviz to `rs_helios`, and add rostopic "/velodyne_points".  
