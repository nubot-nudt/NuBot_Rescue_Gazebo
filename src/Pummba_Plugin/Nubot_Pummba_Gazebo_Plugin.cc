...
using namespace std;
GZ_REGISTER_MODEL_PLUGIN(NubotPummbaGazebo)

NubotPummbaGazebo::NubotPummbaGazebo()
{
...
void NubotPummbaGazebo::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  // This Plugin is placed in the Nubot_Pummba.sdf, _model represent the robot. Get the model.
  GZ_ASSERT(_model, "NubotPummbaPlugin _model pointer is NULL");
  //robot_model_ means the pointer to the Nubot_Pummba robot, related to .sdf file
  robot_model_ = _model;
  //world_model_ means the environments where Nubot_Pummba robot simulating, related to .world file.
  world_model_ = robot_model_->GetWorld();

  GZ_ASSERT(robot_model_, "NubotPummbaPlugin robot_model_ pointer is NULL");
  GZ_ASSERT(world_model_, "NubotPummbaPlugin world_model_ pointer is NULL");

  world_name_ = world_model_->Name();
  robot_name_ = robot_model_->GetName();

  gzmsg << "NubotPummbaGazebo_Plugin: world_model_ name is "
        << world_name_ << std::endl;
  gzmsg << "NubotPummbaGazebo_Plugin: robot_model_ name is "
        << robot_name_ << std::endl;

  if (world_model_->Physics()->GetType() != "ode")
  {
    gzerr << "Tracked vehicle simulation works only with ODE." << std::endl;
   throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed. ODE Problem");
  }

  // Get the robot's .sdf file name.
  robot_sdf_ = _sdf;
  GZ_ASSERT(_sdf, "NubotPummbaGazebo_Plugin robot_sdf_ pointer is NULL");
  ...
  {
    gzerr << "NubotPummbaGazebo_Plugin: <body> tag missing." << std::endl;
    throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed.");
  }

  if (!robot_sdf_->HasElement("left_track"))
  {
    gzerr << "NubotPummbaGazebo_Plugin: <left_track> tag missing."
          << std::endl;
    throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed.");
  }
  ...
  {
    gzerr << "NubotPummbaGazebo_Plugin: <right_track> tag missing."
          << std::endl;
    throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed.");
  }
  ...
  if (body_ == nullptr)
  {
    gzerr << "NubotPummbaGazebo_Plugin: <body> link does not exist."
          << std::endl;
    throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed.");
  }
  else
  {
    gzmsg << "NubotPummbaGazebo_Plugin: Successfully added robot body link "
  ...
  {
    gzerr << "NubotPummbaGazebo_Plugin: <left_track> link does not exist."
          << std::endl;
    throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed.");
  }
  else
  {
    gzmsg << "NubotPummbaGazebo_Plugin: Successfully added left track link "
  ...
  {
    gzerr << "NubotPummbaGazebo_Plugin: <right_track> link does not exist."
          << std::endl;
    throw std::runtime_error("NubotPummbaGazebo_Plugin: Load() failed.");
  }
  else
  {
    gzmsg << "NubotPummbaGazebo_Plugin: Successfully added right track link "
  ...
  if (robot_sdf_->HasElement("left_flipper"))
  {
    ...
    while (flipper)
    {
      ...
      {
        gzerr << "NubotPummbaGazebo_Plugin: <left_flipper> _track link '"
              << flipperName << "' does not exist." << std::endl;
      }
      else
      {
        gtracks[Tracks::LEFT].push_back(flipperLink);
        gzmsg << "NubotPummbaGazebo_Plugin: Successfully added left flipper track"
  ...
  if (robot_sdf_->HasElement("right_flipper"))
  {
    ...
    while (flipper)
    {
      ...
      if (flipperLink == nullptr)
      {
        gzerr << "NubotPummbaGazebo_Plugin: <right_flipper> _track link '"
              << flipperName << "' does not exist." << std::endl;
      }
      else
      {
        gtracks[Tracks::RIGHT].push_back(flipperLink);
        gzmsg << "NubotPummbaGazebo_Plugin: Successfully added right flipper track"
                 "link '" << flipperName << "'" << std::endl;
      }
  ...
  if (robot_sdf_->HasElement("front_left_flipper_j"))
  {
    ...
    {
      gzerr << "NubotPummbaGazebo_Plugin: <front_left_flipper_j> joint '"
    ...
    else
    {
      ...
      gzmsg << "NubotPummbaGazebo_Plugin: Successfully added joint '"
  ...
  if (robot_sdf_->HasElement("rear_left_flipper_j"))
  {
    ...
    if (flipperJoint == nullptr)
    {
      gzerr << "NubotPummbaGazebo_Plugin: <rear_left_flipper_j> joint '"
            << flipperName_j << "' does not exist." << std::endl;
    }
    else
    {
  ...
  if (robot_sdf_->HasElement("front_right_flipper_j"))
  {
    ...
    if (flipperJoint == nullptr)
    {
      gzerr << "NubotPummbaGazebo_Plugin: <front_right_flipper_j> joint '"
            << flipperName_j << "' does not exist." << std::endl;
    }
    else
    {
  ...
  if (robot_sdf_->HasElement("rear_right_flipper_j"))
  {
    ...
    if (flipperJoint == nullptr)
    {
      gzerr << "NubotPummbaGazebo_Plugin: <rear_right_flipper_j> joint '"
            << flipperName_j << "' does not exist." << std::endl;
    }
    else
    {
  ...
  if (!ros::isInitialized())
  {
    ROS_FATAL_STREAM("A ROS node for Gazebo has not been initialized, unable to load plugin. "
                     << "Load the Gazebo system plugin 'libNubotPummbaPlugin.so' in the gazebo_ros package)");
    return;
  }
  ...
  //            "/gazebo/model_states", 100, boost::bind( &NubotPummbaGazebo::model_states_CB,this,_1),
...
#if 0
    reconfigureServer_ = new dynamic_reconfigure::Server<nubot_gazebo::NubotPummbaGazeboConfig>(*rosnode_);
    reconfigureServer_->setCallback(boost::bind(&NubotPummbaGazebo::config, this, _1, _2));
#endif

  // Custom Callback Queue Thread. Use threads to process message and service callback queue
  message_callback_queue_thread_ = boost::thread( boost::bind( &NubotPummbaGazebo::message_queue_thread,this ) );
  //service_callback_queue_thread_ = boost::thread( boost::bind( &NubotPummbaGazebo::service_queue_thread,this ) );
  ...
  //         boost::bind(&NubotPummbaGazebo::update_child, this));
...
void NubotPummbaGazebo::SetBodyVelocity(
...
void NubotPummbaGazebo::SetTrackVelocity(double _left, double _right)
{
  //gzmsg << "NubotPummbaGazebo_Plugin: SetTrackVelocity is calling" << std::endl;
...
void NubotPummbaGazebo::SetTrackVelocityImpl(double _left,
                                             double _right)
{
  //gzmsg << "NubotPummbaGazebo_Plugin: SetTrackVelocityImpl is calling" << std::endl;
...
void NubotPummbaGazebo::SetFlipPose(
...
void NubotPummbaGazebo::UpdateTrackSurface()
{
  auto& gtracks = globalTracks.at(body_);
...
double NubotPummbaGazebo::GetSteeringEfficiency()
{
  return this->steeringEfficiency;
}

double NubotPummbaGazebo::GetTracksSeparation()
{
  return this->tracksSeparation;
}

boost::optional<double> NubotPummbaGazebo::GetTrackMu()
{
  return std::stod(trackMu->GetDefaultAsString());
}

boost::optional<double> NubotPummbaGazebo::GetTrackMu2()
{
  return std::stod(trackMu2->GetDefaultAsString());
}

//void NubotPummbaGazebo::SetSteeringEfficiency(double _steeringEfficiency)
//{
...
void NubotPummbaGazebo::SetLinkMu(const physics::LinkPtr &_link)
{
...
void NubotPummbaGazebo::SetGeomCategories()
{
...
void NubotPummbaGazebo::DriveTracks(/*const common::UpdateInfo &_unused*/)
{
...
ignition::math::Vector3d NubotPummbaGazebo::ComputeFrictionDirection(
...
double NubotPummbaGazebo::ComputeSurfaceMotion(const double _beltSpeed,
...
NubotPummbaGazebo::ContactIterator
NubotPummbaGazebo::ContactIterator::operator++()
{
...
NubotPummbaGazebo::ContactIterator::ContactIterator()
...
NubotPummbaGazebo::ContactIterator::ContactIterator(
    const NubotPummbaGazebo::ContactIterator &_rhs)
{
...
NubotPummbaGazebo::ContactIterator
NubotPummbaGazebo::ContactIterator::begin(
    dBodyID _body, dGeomID _geom1, dGeomID _geom2)
{
...
NubotPummbaGazebo::ContactIterator
NubotPummbaGazebo::ContactIterator::end()
{
  return ContactIterator(true);
}

bool NubotPummbaGazebo::ContactIterator::operator==(
    const NubotPummbaGazebo::ContactIterator &_rhs)
{
...
NubotPummbaGazebo::ContactIterator
NubotPummbaGazebo::ContactIterator::operator++(int /*_unused*/)
{
...
NubotPummbaGazebo::ContactIterator::reference
NubotPummbaGazebo::ContactIterator::operator*()
{
  if (!this->initialized)
    ++(*this);
...
NubotPummbaGazebo::ContactIterator::pointer
NubotPummbaGazebo::ContactIterator::operator->()
{
  if (!this->initialized)
    ++(*this);
...
NubotPummbaGazebo::ContactIterator::pointer
NubotPummbaGazebo::ContactIterator::getPointer()
{
  if (!this->initialized)
    ++(*this);
...
bool NubotPummbaGazebo::ContactIterator::operator!=(
    const NubotPummbaGazebo::ContactIterator &_rhs)
{
  return !NubotPummbaGazebo::ContactIterator::operator==(_rhs);
}


void NubotPummbaGazebo::update_child()
{
...
//  gzmsg << "NubotPummbaGazebo_Plugin: Track_Cmd_CB body_->WorldPose Pos.Y:"
...
void NubotPummbaGazebo::message_queue_thread()
{
  ...
  while (rosnode_->ok())
  {
    ...
    // gzmsg << "NubotPummbaGazebo_Plugin: message_queue_thread is running" << std::endl;
  }
}
